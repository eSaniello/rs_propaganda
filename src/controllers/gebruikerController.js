...
fetch('http://127.0.0.1:3000/api/gebruikers', {
    method: 'GET',
    headers: myHeaders,
    mode: 'cors',
    cache: 'default'
    ...
    .then(data => {
        ...
        if (data.data.length > 0) {
            ...
            data.data.forEach(i => {
                body += "<tr>";
                body += "<td>" + i.gebruiker_id + "</td>";
                body += "<td>" + i.gebruikernaam + "</td>";
                body += "<td>" + i.naam + "</td>";
                body += "<td>" + i.voornaam + "</td>";
                body += "<td>" + i.adres + "</td>";
                body += "<td>" + i.telefoonnummer + "</td>";
                body += "<td>" + i.email + "</td>";
                body += "<td>" + i.type + "</td>";
                body += `<td>
...
function deleteCheck(td) {
    ...
    function deleteGebruiker() {
        ...
        fetch('http://127.0.0.1:3000/api/gebruikers/' + id, {
            method: 'DELETE',
            headers: myHeaders,
            mode: 'cors',
            cache: 'default'
